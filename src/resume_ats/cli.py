...
def extract(
    ...
    try:
        ...
        if output_format == "table":
            ...
            if data.skills:
                skills_panel = Panel(
                    ", ".join(data.skills[:10]) + ("..." if len(data.skills) > 10 else ""),
...
def validate(
    ...
    try:
        ...
        name_ok = expected_name.lower() in pdf_data.name.lower() if pdf_data.name else False
        ...
        position_ok = expected_position.lower() in pdf_data.position.lower() if pdf_data.position else False
...
def setup(
    force: bool = typer.Option(
        False,
        "--force",
        help="Force re-download of dependencies."
    ),
):
